...
function resolveConnector(req, res, next) {
  ...
  if (!service) {
    return res.status(404).json({ error: "Local Git connector not found or not enabled" });
  }
...
router.get("/:connectorId/files", resolveConnector, (req, res) => {
  try {
    ...
    res.status(err.message.includes("not allowed") ? 403 : 500).json({ error: err.message });
  }
});
...
router.get("/:connectorId/commits", resolveConnector, (req, res) => {
  try {
    ...
    const commits = req.localGit.getRecentCommits(count ? Number(count) : undefined);
    res.json(commits);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }

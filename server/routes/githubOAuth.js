...
router.get("/callback", requireOAuthConfig, async (req, res) => {
  ...
  if (!state || !pendingStates.has(state)) {
    return res.send(oauthResultPage(false, null, "Invalid or expired state. Please try again."));
  }
  pendingStates.delete(state);

  if (!code) {
    return res.send(oauthResultPage(false, null, "No authorization code received."));
  }
  ...
  try {
    ...
    if (tokenData.error) {
      return res.send(oauthResultPage(false, null, tokenData.error_description || tokenData.error));
    }

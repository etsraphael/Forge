...
function resolveConnector(req, res, next) {
  ...
  if (!service) {
    return res
      .status(404)
      .json({ error: 'GitHub connector not found or not enabled' })
  }
  req.github = service
  next()
}
...
router.get('/:connectorId/commits', resolveConnector, async (req, res) => {
  try {
    ...
    const commits = await req.github.getRecentCommits(

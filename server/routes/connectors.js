...
router.get("/", (req, res) => {
  ...
  const rows = db.prepare("SELECT * FROM provider_settings ORDER BY created_at ASC").all();
...
router.post("/", (req, res) => {
  ...
  res.status(201).json({ ...row, config: JSON.parse(row.config), enabled: row.enabled === 1 });
});
...
router.patch("/:id", (req, res) => {
  ...
  const newConfig = JSON.stringify(config !== undefined ? { ...config, category: existing.category } : existing);

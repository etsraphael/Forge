...
export class GitHubService {
  ...
  async getFileContent(filePath, ref) {
    ...
    if (Array.isArray(data)) {
      ...
      return {
        type: "directory",
        entries: data.map((e) => ({ name: e.name, type: e.type, path: e.path, size: e.size })),
      };
    }
  ...
  async getTree(treePath = "", ref) {
    ...
    if (!Array.isArray(data)) {
      return [{ name: data.name, type: data.type, path: data.path, size: data.size }];
    }
...
export function getGitHubService(db, connectorId) {
  const row = db.prepare("SELECT * FROM provider_settings WHERE id = ?").get(connectorId);
  if (!row) return null;
 ...
 */
export function getFirstGitHubService(db) {
  const rows = db.prepare("SELECT * FROM provider_settings WHERE enabled = 1 ORDER BY created_at ASC").all();
  for (const row of rows) {
    const config = JSON.parse(row.config || "{}");
    if (config.category === "repository" && row.provider === "github" && config.token && config.repo) {
      return new GitHubService(config.token, config.repo);
    }
  }
  return null;

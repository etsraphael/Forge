...
export class LocalGitService {
  ...
  getRepository() {
    ...
    let totalCommits = 0;
    try {
      totalCommits = parseInt(execGit(this.repoPath, ["rev-list", "--count", "HEAD"]), 10);
    } catch {
      // Empty repo
    }
...
export function getLocalGitService(db, connectorId) {
  ...
  if (config.category !== "repository" || row.provider !== "local-git") return null;
...
export function getFirstLocalGitService(db) {
  ...
  for (const row of rows) {
    const config = JSON.parse(row.config || "{}");
    if (config.category === "repository" && row.provider === "local-git" && config.path) {
      return new LocalGitService(config.path);
    }
  }
  return null;
